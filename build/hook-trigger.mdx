...
description: Set up your REST Hook trigger in the Platform UI with the Settings, Input Designer and API Configuration tabs.
---
## Prerequisites
...
– **Visibility in Editor**: An option to select when this trigger will be shown. _Shown_ is chosen by default. Choose `Hidden` if this trigger should not be shown to users.`Hidden` is usually selected when the trigger is not ready to be used in the integration, or for polling triggers that power [dynamic dropdown](/build/add-fields#dynamic-dropdown) fields.
...
On the Input Designer page, add user [input fields](/build/add-fields) needed by your API to watch for the triggering item.
...
<Frame>
  <img src="/images/0368aeae12e11ec59688d10a7ef69d8c.png"/>
</Frame>

REST Hook triggers are marked as _Instant_ [in the Zap editor](/images/f510859bf90c0e341bc94997a75f9626.png).
...
<Frame>
  <img src="/images/8b7941e3092850bd7edf331cb78b5659.png"/>
</Frame>

If you need to customize the subscription request, select _Switch to Code Mode_ and [write custom JavaScript code](/build/code-mode) to handle the API call and the response parsing as needed.
...
<Frame>
  <img src="/images/44615176b56966a90101067d719b09ad.png"/>
</Frame>

A REST Hook trigger missing a Subscribe or Unsubscribe endpoint, is presented to users as a [Static Webhook](/images/3b35908a6a0c232087b5da807cf9d6fb.png). Static hooks are [not supported in public integrations](/publish/integration-checks-reference#d017---static-hook-is-discouraged), but they could be used if the integration intends to remain private.

### Perform List

This request, usually a GET, is used to collect sample data in the Zap editor, typically prior to a Zap’s activation. This will be used to fetch sample data when users are testing the trigger and mapping fields to their Zap’s subsequent steps. Though optional, not defining a Perform List is a sub-optimal experience for users and is [required for public apps](/quickstart/private-vs-public-integrations).
...
To test the REST Hook trigger, [build a Zap in the editor](/build/test-triggers-actions).

## 5\. Define your output

Define sample data and output fields following [the guide](/build/sample-data).
