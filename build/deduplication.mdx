...
description: "Zapier automatically deduplicates incoming trigger data for your integration, so that Zaps do not run multiple times on the same data. Consider the following requirements for your “New Item” and “Updated Item” triggers to work as users expect."
---
...
[Polling](/build/trigger) usually returns many results, most of which Zapier has seen before. Since a Zap should not trigger multiple times when an item in your app exists in multiple distinct polls, the data must be deduplicated.
...
If your API cannot order its results in reverse-chronological order, you can use [Code Mode](/build/code-mode) to make additional requests, if necessary.
...
When adding additional requests to your custom code, all requests and processing code for a trigger must [finish within 30 seconds](/build/operating-constraints#timeouts-triggers). It is not recommended to attempt to fetch all pages of results.

If the items your API returns do not have an `id` field or you’re adding an Updated Item trigger, you will use [Code Mode](/build/code-mode) to modify the API response.

## Custom primary keys

In older, [legacy Zapier Web Builder apps](/manage/versions-legacy), Zapier guessed what fields made a unique key if `id` wasn’t present. It is now required that you define one or more fields as the primary key.
