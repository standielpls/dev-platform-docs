...
Platform UI does not currently support other grant types. If your integration requires a different OAuth v2 grant type, you’ll need to use another supported authorization type with Zapier such as [Session](/build/sessionauth) or [API Key](/build/apikeyauth).

If your integration requires OAuth v1 authentication, use the [Platform CLI](/quickstart/cli-tutorial) instead of Platform UI.
...
*   Two types of fields are available when building an OAuth v2 input form. Standard Fields, work much like other form fields with Zapier’s [input form](/build/field-definitions) in triggers and actions. [Computed Fields](/build/computed-fields) make sure specific fields are returned by your app’s authentication API call response.
...
*   If your Access Token and Refresh Token requests don’t return the tokens at the top level, use [Code Mode](/build/code-mode) to modify the response so that the tokens are available at the top level. It is not possible to store an object with nested keys from the response.
