...
\*Zapier searches automatically return only the first result in the response. To return multiple results, [return the set of results](/build/response-types) as an array of objects under a descriptive key.
