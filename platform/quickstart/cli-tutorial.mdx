...
description: "This tutorial walks you through the process of building, testing, and pushing an example app to Zapier using Platform CLI. We’ll use a mock API for recipes in this tutorial, but for production Zapier apps, you’d want to connect to a real API."
---
...
From the Zapier CLI, run: `zapier team:add user@example.com admin` to invite a team member [as an admin](/manage/add-team#admin) for the app.

OR

`zapier team:add user@example.com collaborator` to invite a team member [as a collaborator](/manage/add-team#collaborator) for the app.
...
Public integrations or private integrations with more than 5 users **require** a new version to make any edits. It is still recommended to make changes for private integrations with fewer than 5 users in a new version as well and [consider Zapier’s best practices](/manage/planning-changes) when doing so.
