...
*   **Zapier**: An integration platform that connects over 7,000+ web apps and APIs together into [multi-step, automated, customizable workflows](/quickstart/how-zapier-works). When an event happens in one app, Zapier can tell another app to perform (or do) a particular action.
...
*   **Trigger**: The app integration event that [starts a Zap when new or updated data is added](/build/trigger) to your app, either by Zapier polling a specific API endpoint to check for new data or via a notification REST webhook from your app that pushes new data to Zapier.
*   **Action**: The app integration step that [finds, creates, or updates data](/build/action) in your app using a GET, PUT or POST request to your API using data provided when a Zap is triggered by an event in another app.
...
*   **Beta**: If your app is published in the Zapier App Directory, it will [remain in Beta](/publish/public-integration#5-beta-phase) for 90 days to indicate its newer status to end users.
