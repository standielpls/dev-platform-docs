---
title: Action
description: "Every Zap starts with a single trigger that watches for new or updated data, starting the user’s workflow. Action steps then make use of that data."
---

Zapier actions create or update a single item in your app through API calls that include multiple details from user customized [input fields](/build/add-fields).
<Frame>
  <img src="/images/57f28534d180f2a642ebe0be2e236c32.png" alt="Zapier Action Visual Builder"/>
</Frame>
...
Most Zapier integrations should at a minimum include create actions to let users add items to their app automatically. Common actions by app category [here](/quickstart/must-have-triggers-and-actions) should be used for inspiration when building your app.
...
The [output returned](/build/response-types) by a _create_ should be an object containing individual fields about the item that was created such as IDs, details about the new item including a link if possible, and any other useful data about the record. Do not return just a `success` message.
...
_Search_ actions find existing items in an app and can optionally be paired with _create_ actions to [add a new item](/build/search-or-create) if the search does not return a result.
...
The [output returned](/build/response-types) by a _search_ should be a JSON-formatted array sorted with the best match first. Only the first item will be returned. For no match found, a `200` with an empty array must be returned. If your API returns a `404` error for searches without results, add custom code to your API call to replace it with an empty array.
