...
Then it uses the trigger’s `perform` method to build a [deduplication table](/build/deduplication) of records, so that the Zap will not run for existing records. The payload returned from this request must also be less than 20 MB.
