...
**Important**: integrations for prohibited apps will be removed from the platform. Integrations for restricted apps or that do not meet all requirements will be allowed to remain [private](/quickstart/private-vs-public-integrations).
...
*   If you prefer your integration to be accessible to a limited group of users, or if the Zapier integration review requirements do not align with your needs, you might want to consider maintaining your integration as private. Learn more about [private vs public integrations](/quickstart/private-vs-public-integrations).
...
<CardGroup cols={3}>
 ...
 <Card title="Ownership and Permissions" icon="user-shield" href="/publish/integration-publishing-requirements#3-ownership-and-permissions" horizontal/> <Card title="Data Security and Privacy" icon="lock" href="/publish/integration-publishing-requirements#4-data-security-and-user-privacy" horizontal/>
 <Card title="Quality and Functionality" icon="circle-check" href="/publish/integration-publishing-requirements#5-quality-and-functionality"horizontal /> <Card title="Integration Listing" icon="list" href="/publish/integration-publishing-requirements#6-integration-listing" horizontal/>
...
Do not hard code credentials such as API Keys, Client IDs, Client Secrets, etc. into your integration. Use the appropriate section in the Platform UI (e.g. Application Credentials) or use [environment variables](/build/env) instead.
...
For more information and guidance, refer to [Integration Build Guidelines](/publish/integration-build-guidelines) and [Test and monitor your integration in your Zapier account](/build/test-monitoring).
...
Authentication credentials are only requested from users via the [Authentication](/build/auth) configuration. Integrations that request authentication credentials to be entered in other configurations, such as triggers, actions, or searches, will not be published.

### 5.6 Integration has a valid connection label

[Connection labels](/build/connection-label) are optional but can help users identify which account each [app connection](https://help.zapier.com/hc/en-us/articles/8496258785421-Connect-your-app-accounts-to-Zapier) uses. Suitable values include account name, email address, and name.
...
Triggers, actions, searches, and input fields should be named according to platform conventions. For more information, refer to [Trigger Copy](/publish/integration-build-guidelines#trigger-design-and-copy), [Trigger input fields](/publish/integration-build-guidelines#trigger-input-fields), [Action Copy](/publish/integration-build-guidelines#action-design-and-copy), [Action input fields](/publish/integration-build-guidelines#action-input-fields), [Search Copy](/publish/integration-build-guidelines#search-design-and-copy), and [Search input fields](/publish/integration-build-guidelines#search-input-fields).
...
Integrations must have at least one [admin team member](/manage/add-team) with an email address from the application or API’s top-level domain, or the company owning the API used in the integration. Add new team members to an integration by following this guide: [Invite team members to your integration](/manage/add-team).
