...
description: "This guide provides instructions on editing and maintaining existing scripting methods for legacy web builder integrations that have been converted to either the Platform UI or Platform CLI."
---
## Introduction


> **Note**: This guide isn’t for new integrations built in Platform CLI or Platform UI. For new integrations, use the [Platform UI](/quickstart/ui-tutorial) or the [Platform CLI](/reference/cli-docs) to build an integration in code.


If you are creating new functionality, check maintaining your converted integration for the best way forward.

Much like [Code Mode](/build/code-mode), Zapier’s Web Builder Scripting was the previous way to allow you to directly manipulate the requests and responses that are exchanged between your app’s API and Zapier. It continues to be supported using both the Platform UI and CLI.
...
<Frame>
  <img src="/images/e776d2b6349552e6347ca2757f4d06c1.png"/>
</Frame>
...
See [bundle.request](/reference/legacy-scripting#prepared-request-via-bundlerequest).
...
It runs after Zapier receives a response from the polling URL. Can parse the response to format the data that enters Zapier or throw an [exception](/reference/legacy-scripting/#available-exceptions). Zapier will automatically throw for status codes 4xx and 5xx after the method runs.
...
It runs in place of pre\_poll and post\_poll. When you get a bundle, you are expected to make the request and return a list of results or throw an [exception](/reference/legacy-scripting/#available-exceptions). Zapier will **not** throw for status codes like 4xx and 5xx automatically!
...
Learn more about [bundle.request](/reference/legacy-scripting#bundle-details).
...
Learn more in [bundle.request](/reference/legacy-scripting#bundle-details).
...
It runs after Zapier receive a response from the consuming call and can parse the response to format the data that enters Zapier or throw an [exception](/reference/legacy-scripting/#available-exceptions). Zapier will automatically throw for status codes 4xx and 5xx after the method runs.
...
Runs after the response for custom fields is received. Can parse the response to format the data that enters Zapier or throw an [exception](/reference/legacy-scripting/#available-exceptions). Zapier will throw for status codes 4xx and 5xx after the method runs automatically.
...
Runs in place of pre_write and post\_write. You get a bundle and are expected to make the request and return the appropriate response or throw an [exception](/reference/legacy-scripting/#available-exceptions). Zapier will **not** throw for status codes like 4xx and 5xx automatically! \_Note:_ If the action occurs as part of a search-or-create Zap, the output of this method is _not_ exactly what the user sees. In that case, the action will be followed up with a request to fetch the written record, and Zapier will present the user with the output from that follow-up request. If you need to modify the returned data in that scenario, use `_post_read_resource`.
...
Runs after the response for custom response fields is received. Can parse the response to format the data that enters Zapier or throw an exception [exception](/reference/legacy-scripting/#available-exceptions). Zapier will throw for status codes 4xx and 5xx after the method runs automatically.
...
Runs after the response for custom search result fields is received. Can parse the response to format the data that enters Zapier or throw an [exception](/reference/legacy-scripting/#available-exceptions). Zapier will throw for status codes 4xx and 5xx after the method runs automatically.
...
Runs after Zapier do the request to read an individual resource. Use to modify the data returned or throw an [exception](/reference/legacy-scripting/#available-exceptions). Zapier will throw for status codes 4xx and 5xx after the method runs automatically.
...
Runs in place of pre\_read\_resource and post\_read\_resource. You get a bundle and are expected to make the request and return the appropriate response or throw an [exception](/reference/legacy-scripting/#available-exceptions). Zapier will **not** throw for status codes like 4xx and 5xx automatically!
...
Modify the response from the access token endpoint or throw an [exception](/reference/legacy-scripting/#available-exceptions). Zapier will throw for status codes 4xx and 5xx **before** the method runs automatically.
...
> Please note that request will be done via the [requests library for Python](http://docs.python-requests.org/en/master/user/quickstart/#make-a-request) which is different from [z.request()](/reference/legacy-scripting#making-outbound-requests-zrequest). You cannot set an `auth` property (e.g. for Basic Auth). Instead provide the exact `params` and `headers`.
...
> **Note**: This guide is for Zapier’s legacy web builder. If you use Platform UI, Zapier expects [standard HTTP errors](/build/auth#common-authentication-error-messages) to be thrown. If you use Platform CLI, learn more in our [CLI error handling docs](https://zapier.github.io/zapier-platform-cli/#error-handling).
...
> Heads up! This code is only valid for the v2 platform, and is incompatible with today’s [Platform CLI and Platform UI](/quickstart/ui-vs-cli).
...
[Previous: Custom Actions and API Requests Actions](/reference/custom-actions-api-requests) [Next: Changelog](/reference/changelog)
